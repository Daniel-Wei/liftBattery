...
`RunId` is persisted on the Table Storage job row before enqueueing and is unique to one job run. Re-sending that same run during enqueue recovery keeps the same `MessageId`, so Service Bus duplicate detection can suppress an uncertain duplicate send. A new run after cancellation or failure receives a new `RunId` and therefore cannot be mistaken for the previous job. The queue message carries the same `RunId`, and Service Bus `CorrelationId` is set to that value, so the run can be followed through the producer logs, Service Bus message metadata, consumer logs, and Table Storage job row. The message does not carry `DataVersion`; SQL and the durable Job row own version semantics.

Runtime flow:

1. `CreateTrendReport` validates the request and reads the current SQL `DataVersion`. A missing version means no report source data has ever been stored, so submission returns 422 without creating a Job.
2. Submission does not load source rows or persist a snapshot. It creates a lightweight Job whose captured `DataVersion` is part of the request identity.
...
4. Each user partition contains one fixed `active-job` lease. For a new job, the repository creates the dedup row, lightweight job row, and lease in one Table transaction. The repository reads the lease first: an active job with identical parameters is returned, while an active job with different parameters produces 409. A missing leased job or mismatched RunId is treated as storage corruption. Explicit cancellation and every terminal worker transition update the job and delete its matching lease atomically.
...
7. Each valid delivery verifies `JobId`, `RunId`, and the current non-terminal status. It then performs version check 1 by reading SQL `DataVersion`, Training, and Pre-check together in one snapshot-isolation transaction. A matching generation becomes the attempt's immutable in-memory snapshot; a mismatch conditionally supersedes the Job. Moving `EnqueuePending` or `Queued` to `Processing` is best-effort display state, so redelivery may recompute an existing `Processing` Job.
8. Immediately before completion, the worker performs version check 2 against the current SQL `DataVersion`. If unchanged, it uploads the result to a content-addressed Blob and conditionally publishes `ResultBlobName` in the terminal Table transaction. Status checks and ETag ensure that only the first terminal writer wins and `Completed`, `Failed`, `Cancelled`, or `Superseded` cannot be overwritten.
